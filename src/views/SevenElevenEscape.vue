    ...
    <template v-if="phase==='store' || phase==='win'">
      ...
      <div class="store-scene">
        ...
        <div class="zone mug-shelf" @mouseenter="hovering=true" @mouseleave="hovering=false" @click.stop="openModal('mugs')">
          ...
          <div class="mug-row">
            <div class="mug-3d" v-for="mug in mugs" :key="mug.letter + mug.under">
              <div class="takeaway-cup" :style="{'--seven-color': mug.sevenColor}">
                <div class="cup-lid"></div>
                <div class="cup-body">
                  <div class="cup-seven">7</div>
                </div>
                <div class="cup-base"></div>
              </div>
              <div style="display:none">{{ mug.under }}</div>
            </div>
          </div>
      ...
      <div class="modal-panel orange">
        ...
        <div class="mug-grid">
          <div class="mug-detail" v-for="mug in mugs" :key="mug.letter">
            <div class="takeaway-cup large" :style="{'--seven-color': mug.sevenColor}">
              <div class="cup-lid"></div>
              <div class="cup-body">
                <div class="cup-seven">7</div>
              </div>
...
// Mugs data — take away cups with 7-Eleven colors
const mugs = ref([
  { sevenColor: '#2ecc71', cupColor: '#f5f0e8', under: 'S', letter: 'S' },
  { sevenColor: '#e8212a', cupColor: '#f5f0e8', under: 'T', letter: 'T' },
  { sevenColor: '#f7941d', cupColor: '#f5f0e8', under: 'A', letter: 'A' },
  { sevenColor: '#2ecc71', cupColor: '#f5f0e8', under: 'F', letter: 'F' },
  { sevenColor: '#e8212a', cupColor: '#f5f0e8', under: 'F', letter: 'F' },
]);
...
/* === MUG SHELF === */
.mug-shelf {
  left:8%;bottom:20%;width:200px;height:100px;
}
.mug-shelf-board {
...
.takeaway-cup:hover { transform:translateY(-5px) scale(1.08); }
.takeaway-cup.large { width:58px; }

.cup-lid {
  width:115%;
  height:8px;
  background:linear-gradient(180deg,#ddd8ce,#cac4ba,#b8b2a8);
  border-radius:40% 40% 20% 20% / 60% 60% 30% 30%;
  border:1px solid rgba(0,0,0,0.15);
  position:relative;
  z-index:2;
  box-shadow:0 -1px 3px rgba(255,255,255,0.3) inset, 0 2px 4px rgba(0,0,0,0.3);
}
.cup-lid::before {
  content:'';
  position:absolute;
...
.takeaway-cup.large .cup-lid { height:13px; }

.cup-body {
  width:100%;
  flex:1;
  min-height:44px;
  background:linear-gradient(160deg,#faf6ee 0%,#f2ece0 50%,#e8e2d4 100%);
...
.takeaway-cup.large .cup-body { min-height:72px; }
...
.cup-seven {
  ...
  text-shadow:1px 1px 0 rgba(0,0,0,0.12), 0 0 8px rgba(0,0,0,0.08);
  position:relative;
  z-index:1;
  margin-top:3px;
  letter-spacing:-1px;
}
.takeaway-cup.large .cup-seven { font-size:46px; }

.cup-base {
  width:70%;
  height:6px;
  background:linear-gradient(180deg,#cac4ba,#b8b2a8);
  border-radius:0 0 5px 5px;
  border:1px solid rgba(0,0,0,0.12);
  border-top:none;
  box-shadow:0 3px 6px rgba(0,0,0,0.4);
}
.takeaway-cup.large .cup-base { height:10px; }
...
/* Mug puzzle view in modal */
.mug-grid {
  display:flex;gap:18px;justify-content:center;margin:20px 0;align-items:flex-end;
}
.mug-detail {
  text-align:center;cursor:pointer;transition:transform .2s;
  display:flex;flex-direction:column;align-items:center;
}
.mug-detail:hover { transform:translateY(-6px); }

.mug-d-under {
  margin-top:8px;font-family:'Share Tech Mono',monospace;font-size:11px;
